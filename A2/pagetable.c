...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		struct frame *vic_frame = &coremap[frame];
		pgtbl_entry_t *pte = vic_frame->pte;

		// If frame is dirty, write to swap
		// Update pagetable accordingly. Frame is now invalid, swapped, and not modfied
...
char *find_physpage(addr_t vaddr, char type) {
	...
	int entry_valid = p->frame & PG_VALID;
	int entry_on_swap = p->frame & PG_ONSWAP;

	if (!(entry_valid) && (entry_on_swap)){
		miss_count++;
	...
	}else if (!(entry_valid) && !(entry_on_swap)){
		miss_count++;
		int frame = allocate_frame(p);
