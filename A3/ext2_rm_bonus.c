...
int main(int argc, char *argv[]) {
    ...
    }else if ((inode != NULL) && (inode->i_mode & EXT2_S_IFDIR) && (!flag)){
    	printf("Error: Use flag -r to remove directory.\n");
    	return EISDIR;
    
    //if not a directory, remove file
    }else if (!(inode->i_mode & EXT2_S_IFDIR)) {
    	remove_file(dir);
    	return 0;

    //if a directory, and flag set, remove directory
    }else if (flag && (inode->i_mode & EXT2_S_IFDIR)){
    	remove_dir(dir);
    }
